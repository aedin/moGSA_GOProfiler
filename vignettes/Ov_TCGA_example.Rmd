---
title: "Multi-dataset Exploratory and Pathway data analysis of TCGA Ovarian Cancer Data"
vignette: >
...
TCGA  [TCGA data portal here](https://tcga-data.nci.nih.gov/tcga/) data were downlaoded  from the firehose data [firehose_get](https://confluence.broadinstitute.org/display/GDAC/Download) pipeline. Code to downlaod and generate the data is below
...
# Integrated Exploratory data analysis: Finding cluuters in the data
...
```{r se.moa}
require(mogsa)
se.moa<-moa(lapply(se, exprs), proc.row = "center_ssq1", w.data = "inertia" )
```
...
moa returns an S4 object with class "moa"
```{r resmoc}
class(se.moa)
slotNames(se.moa)
```
...
To extract any slot from an object of class moa, use the function slot (* we will improve this in a future release of the package)
...
To extract the global scores (of the observations) for the first 5 tumors on the first 5 components.
...
To extract the loadings or coefficients of the features of each dataset, we use moaCoef. This returns a list of length 2.  The first object, is a data.frame of the concatentaed feature loadings. Therefore the nrow is equal to the total union of all features.  Features are renames rownames_dataset.  

```{r extractCoefs}
features<-moaCoef(se.moa)
names(features)
dim(features$coefMat)
features$coefMat[1:10,1:4]
```

The second object in the list is called *nonZeroCoef* and  is a list of data.frame to list the non-zero coefficient variable in each of loading vectors and data sets.
...
Here we show with an example how to use MOGSA to integrate and annotate multiple omics data.
...
```{r mogsaBasicRun}
mgsa1 <- mogsa(S2, sup=annotS2, nf=5,
...
The function *mogsa* returns an object of class *mogsa*. This information could be 
extracted with function *getmgsa*. First, we want to know the variance explained by each PC
...
The main result returned by *mogsa* is the gene set score (GSS) matrix. The value in the matrix indicates the 
...
Figure 2 shows the gene set score matrix returned by *mogsa*.
...
For example, if we want to select the top 20  most significant gene sets and plot them in heatmap, we do:
...
Next, in order to know the most influential genes in this gene set `r gs1`. We call the function \Rfunction{GIS}:
...
In figure 5, the bars represent the gene influential scores for genes. Genes from different platforms are shown in different colors. The expression of genes with high positive GIS more likely to have a good positive correlation with the gene set score. In this example, the most important genes in the gene set are mostly cytokines (IL-21R, IL7R etc)
